...
# Run BaGetter on Scaleway

Use [Scaleway](https://www.scaleway.com) to host BaGetter. You can store metadata on a [Managed Database](https://www.scaleway.com/en/database/) and upload packages to [Scaleway Object Storage](https://www.scaleway.com/en/object-storage/), which is S3-compatible.
...
Scaleway Object Storage is S3-compatible. Create a bucket and an API key from the [Scaleway console](https://console.scaleway.com), then update `appsettings.json`:
...
Scaleway Managed Databases support PostgreSQL natively. Create a database instance, then update `appsettings.json`:
...
```json
{
    ...
    "Database": {
        "Type": "PostgreSql",
        "ConnectionString": "Host=<instance-ip>;Port=<port>;Database=bagetter;Username=<user>;Password=<password>;Ssl Mode=Require;Trust Server Certificate=true"
    },
...
Create a MySQL managed database instance, then update `appsettings.json`:
...
```json
{
    ...
    "Database": {
        "Type": "MySql",
        "ConnectionString": "Server=<instance-ip>;Port=<port>;Database=bagetter;User=<user>;Password=<password>;SslMode=Required"
    },
